...
llvm::Value* CompilerState::getCharArrayElement(llvm::Value* arr, llvm::Value* index) {
...
void CompilerState::setCharArrayElement(llvm::Value* arr, llvm::Value* index, llvm::Value* theChar)  {
    auto* type = llvm::Type::getInt8Ty(context);
    auto elemPtr = builder.CreateGEP(type, arr, index);
...
void CompilerState::return0FromMain()  {
    builder.CreateRet(getConstInt(0));
}

llvm::BasicBlock* CompilerState::createBasicBlock(const std::string& s, llvm::Function* function)  {
...
[[nodiscard]] llvm::Value* CompilerState::generateCallReadCharFunction()  {
    return builder.CreateCall(module.getFunction("readChar"), {});
}

llvm::Value* CompilerState::CreateAdd(llvm::Value* lhs, llvm::Value* rhs, const std::string& name)  {
    return builder.CreateAdd(lhs, rhs, name);
}

llvm::BasicBlock* CompilerState::createBasicBlock(const std::string& s)  {
    return createBasicBlock(s, getCurrentFunction());
}

void CompilerState::finalizeAndPrintIRtoFile(const std::string& outPath)  {
    return0FromMain();
    std::error_code EC;
